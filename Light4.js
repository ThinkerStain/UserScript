...
(function() {
    ...
    function f_addContent() {
        ...
        for (let i = 0; i < srchHere.length; i++) {
            ...
            ///////////////////////////////////
            let newFhosts = /(23(A(FIP|ZOV)|CHER|GR(IG|KL)|ILSK|LVOV|MIHA|NOVO|S(EV|MOL|TAV)|UBIN)|A(DY*G*|ST)|(AFIP|CHER|GRKL|SEV)23|DAG*|ING*|K(LM*|BR*|R*DA*|C(R|H))|NZR|R(ND|o|OS|ST|H)|S(T(A|V)|SI|V*O)|V[LG]G)[\-_][a-zA-Z\d\-\._]+[\/\d\b\s\-\|a-z:;#_=|&gt;]*(\/)*\d+\/\d+(\/\d+)*/gi; // \(\)
            ///Собираем массив из строк где есть регионы, это хостней и брасы
            var nfHost = nCell.innerText.match(newFhosts);
            if (nfHost != null) {
                allHostsSPP = allHostsSPP.concat(nfHost);
            } //Находим все хостнеймы
            var nfBras = nCell.innerText.match(fBras);
            if (nfBras != null) {
                allBrass = allBrass.concat(nfBras);
            } //Находим все Брасы
            ///alert(allBrass+"    111");
            ///////////////////////////////////
        }
        ...
        for (let i = 0; i < allRegions.length; i++) {
            allRegions[i] = allRegions[i].split('\-', 1); //Оставляем первую часть до "-" что бы узнать регион
        }
